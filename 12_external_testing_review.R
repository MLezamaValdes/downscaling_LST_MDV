...
#
  ...
  ex_test_1to3 <- lapply(seq(3), function(j){
    ...
    pred_df <- cbind(testlist[[j]], pred)
    head(pred_df)
    
    saveRDS(pred_df, paste0(predoutpath, "prediction_df_", method, "_test", j, ".RDS"))
    ...
    pred_df$month <- substring(pred_df$ymo,6,7)

    pred_df$month <- as.factor(pred_df$month)

    p <- ggplot(pred_df, aes(Landsat,pred, color=month))+
  ...
  ggsave(paste0(figurepath, "external_eval_new_", method, "_",opts[i], "_by_month.png"), 
