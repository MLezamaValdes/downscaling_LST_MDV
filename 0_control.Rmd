...
```{r}
# LST error
err <- projectRaster(em_err_anta$Error, crs(eta_res))
...
mapview::mapview(err, na.color="#00000000")
...
aqua <- allscenesinstacks[allscenesinstacks$MODMYD==2,]
summary(aqua$timediff)*60
```
...
bt <- read.csv2(paste0(cddir, "blocktest_samples_test.csv"))
blocktestsample <- bt$x 

testblock <- blockmask
testblock[testblock %in% blocktestsample & !is.na(testblock)] <- 200 #test
testblock[testblock !=200 & !is.na(testblock)] <- 0 #train

tbt <- table(testblock[])

fulln <- 13353603 + 8959209 
8959209/fulln
...
```{r}
cf <- "D:/downscaling_after_talk/clean_data/comp_comp_files/"
cff <- list.files(cf, full.names=T)


td <- lapply(seq(cff), function(i){
  read.csv2(cff[i])
})

tdc <- do.call("rbind", td)


write.csv2(tdc, "D:/downscaling_after_talk/clean_data/comp_comp_files/all_timediff_comp.csv" )

mean(tdc$timediff)*60

hist(tdc$timediff, breaks=40)

mean(tdc$timediff[tdc$MODMYD==1])*60
mean(tdc$timediff[tdc$MODMYD==2])*60
...
test1LHS <- read.csv2(paste0(trainvalidpath, "validation_1_LHS_150000.csv"))
test2LHS <- read.csv2(paste0(trainvalidpath, "validation_2_LHS_150000.csv"))
test3LHS <- read.csv2(paste0(trainvalidpath, "validation_3_LHS_150000.csv"))
...
test1 <- read.csv2(paste0(trainvalidpath, "validation_1_rand_150000.csv"))
test2 <- read.csv2(paste0(trainvalidpath, "validation_2_rand_150000.csv"))
test3 <- read.csv2(paste0(trainvalidpath, "validation_3_rand_150000.csv"))

```


```{r}
min(test1$dem) - min(test1LHS$dem)
max(test1$dem) - max(test1LHS$dem)

min(test2$dem) - min(test2LHS$dem)
max(test2$dem) - max(test2LHS$dem)

min(test3$dem) - min(test3LHS$dem)
max(test3$dem) - max(test3LHS$dem)
...
Application to one single scene: 

```{r}
aoadir <- "D:/downscaling_after_talk/aoa/results_split/"

AOA <- raster(paste0(aoadir, "AOA_MYD11_L2.A2018316.1350.006_cat.grd"))

AOA_m <- mask(AOA,aoi)
mapview(AOA_m, na.color="#00000000")
writeRaster(AOA_m, paste0(aoadir, "AOA_masked_MYD11_L2.A2018316.1350.006_cat.grd"))
```

```{r}
s6 <- raster(list.files("D:/downscaling_after_talk/aoa/split_rasters/", pattern="split_6.tif",full.names = T))
